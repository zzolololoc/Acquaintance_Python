...
Задание 2. Генерация
Что нужно сделать
Пользователь вводит целое число N. Вам необходимо написать программу,
которая генерирует список из чисел от 0 до N (не включая N). Например, если N — это 5, то нужно работать со списком 0, 1, 2, 3, 4.
...
#вообще я вот так решил изначально,но потом перечитал задание и сделал как ниже
# for idx,numbers in enumerate(list_numbers):
#     if idx % 2 == 0:
#         list_numbers[idx] = list_numbers[1]
#     else:
#         list_numbers[idx] = list_numbers[idx] % 5



print([1 if i % 2 == 0 else i % 5 for i in range(int(input("Введите длину списка: ")))])